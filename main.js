    ...
    if (index === -1) {
        ...
        if (previousDayTides !== null) {
            minutesFromPrevDay =
                ...
                getTotalMinutesOfTide(
                    previousDayTides[previousDayTides.length - 1]
                );
        }
...
const init = () => {
    loadJSON().then((response) => {
        ...
        const previousDayTides =
            currentDay > 1 ? response.dias[currentDay - 2].mareas : null;
        const nextDayTides =
            currentDay < response.dias.length
                ? response.dias[currentDay].mareas
                : null;
        ...
        for (let i = -1; i < dayTides.length; ++i) {
            const tideElement = document.getElementById(
                'marea' + (i + 1).toString()
            );
            if (i >= 0) {
